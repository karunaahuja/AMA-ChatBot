...
tf.app.flags.DEFINE_string("train_dir", "/home/ubuntu/checkpoints_ubuntu", "Training directory.")
...
tf.app.flags.DEFINE_integer("beam_size", 7,
                            "Size of beam search window.")
...
_buckets = [(5, 10), (10, 15), (20, 25), (40, 50),(55,65), (70,90), (90, 120) ]

logging.basicConfig(filename='new_3layers_lr_0_1_ubuntu.log',level=logging.DEBUG)
  ...
  with tf.Session() as sess:
    ...
    if beam_search:
      ...
      while sentence:
        ...
        recos = set()
        logging.debug ("Replies (Beam Search) --------------------------------------->")
        for kk in range(beam_size):
          ...
          rec = " ".join([tf.compat.as_str(rev_fr_vocab[output]) for output in foutputs])
          if rec not in recos:
            recos.add(rec)
            logging.debug (rec)
              # If there is an EOS symbol in outputs, cut them at that point.
        #if EOS_ID in output:
         # output = output[:outputs.index(EOS_ID)]
          
        #logging.debug ("Replies (Greedy Search) --------------------------------------->")
        #logging.debug(" ".join([tf.compat.as_str(rev_vocab[output]) for output in outputs]))
          #logging.debug("> ", "")
          #sys.stdout.flush()
          #sentence = sys.stdin.readline()
     # ppl_test = math.exp(float(test_loss)) if test_loss < 300 else float("inf")
